...
- `GET https://www.coveragecat.com/api/consumer/mcp` returning `405 Method Not Allowed` is expected; direct MCP transport uses `POST` JSON-RPC.
- Using `https://www.coveragecat.com/.well-known/mcp.json` as the direct transport URL can fail initialize, because that URL is discovery metadata rather than the transport endpoint.
- After adding the MCP server in local Codex, restart the session and check `/mcp` before assuming the tools failed to load.
- If local Codex delegated OAuth for `https://www.coveragecat.com/api/agent/mcp` fails because the host rejects the local callback URL, keep shopper flows on the consumer MCP and use the delegated operator MCP only from a bearer-token setup or an OAuth host whose callback policy is already compatible.

For umbrella consumer handoffs specifically, do not open with a questionnaire on a cold start. Call `/api/consumer/umbrella/prefill` first with the fullest estimate you can justify, prefer canonical umbrella keys such as `address.street`, `address.city`, `address.state`, `address.zip`, `license_state`, `license_number`, `net_worth_numeric`, `required_coverage_e_limit`, and `vehicle_count` or `umbrella_details.motorized_vehicle_count`, and do not send `vehicles` rows before submission; if Markel later needs actual vehicle details, collect them only in the post-select bind continuation. Keep the user out of the loop until `/api/intake/:uid/issues` reaches `ready_for_submission`, or until it returns a staged review plus bundled shopper-detail `next_question`. On every pre-submit user-facing turn, say explicitly that the application is not submitted yet and Coverage Cat has not received a submitted application yet. On that staged review turn, show the assembled facts and ask only for the explicitly missing shopper-owned details in one plain-English message. Keep already reviewed estimated facts on the review card and invite corrections in the same reply instead of asking the shopper to re-key them. When you list gathered details or remaining items, render them as short labeled bullets or sections rather than a prose paragraph. If any shown value is estimated, mark that bullet or value with `*`, include `* = estimated` once above and once below the list, and do not prefix every estimated line with `[Estimated]`. When Coverage Cat asks for it, that missing-details bundle can include full name, email, phone, full address, birthday, marital status, driver's license state, driver's license number, net worth, and corrections to any estimated facts. If marital status means Coverage Cat still needs spouse or partner details, keep spouse full name, spouse date of birth, spouse driver's license state, and spouse driver's license number in that same single review turn instead of opening a second follow-up. If the shopper gives a birthday in normal US `MM-DD-YYYY` form, normalize it to `YYYY-MM-DD` before patching Coverage Cat. Then submit from that same review turn, and only ask for soft-credit consent later if `/select` requires it after the user picks an offer. When Coverage Cat returns multiple offers, present the alternatives instead of collapsing the result to only the recommended default. Render the offers as a single markdown table with columns `Carrier | Coverage limit | Annual price | Min. auto limits | Notes` (recommended offer first, `(Recommended)` in that row's Notes column). Format prices as `$941/yr` and coverage limits as `$2M` / `$3M`. Do not restate offers in prose. Immediately below the table, render a `**Next steps to purchase:**` bulleted checklist covering: picking an offer, confirming Min. auto limits when shown, uploading current home and auto declarations pages when any offer is Monoline or Markel (about 5-10 minutes to secure checkout after upload), and saying `Yes` to the soft-credit-pull consent that Coverage Cat asks for after selection — never bury these steps in a paragraph. For Monoline and Markel, keep the post-choose loop in chat too: after the user selects the offer, upload any required home and auto declarations through `/api/intake/:uid/attach`, then keep polling `/api/intake/:uid/issues`; once those uploads are on file, `documents_pending_review` usually advances to `payment_needed` with a secure `payment_url` within about 5-10 minutes while declarations verification continues in parallel.
